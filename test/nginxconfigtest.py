...
from meresco.distributed.failover.nginxconfig import WRITABLE, READABLE
from os import stat
...
class NginxConfigTest(SeecrTestCase):
    ...
    def testBaseOnWritable(self):
        ...
        n1 = NginxConfig(type='api', name='name', nginxConfigDir=self.tempdir, minVersion=VERSION, untilVersion=VERSION_PLUS_ONE, flags=[WRITABLE])
        mustUpdate, sleeptime = n1.updateConfig(config=config, services=services)
        self.assertTrue(mustUpdate)
        self.assertEquals(20, sleeptime)
...
""" % usrSharePath, open(join(self.tempdir, 'name.frontend.conf')).read())
    ...
    def testBaseOnBothWritableAndReadable(self):
        ...
        n1 = NginxConfig(type='api', name='name', nginxConfigDir=self.tempdir, minVersion=VERSION, untilVersion=VERSION_PLUS_ONE, flags=[READABLE, WRITABLE])
        mustUpdate, sleeptime = n1.updateConfig(config=config, services=services)
        self.assertTrue(mustUpdate)
        self.assertEquals(20, sleeptime)
        self.assertTrue(isfile(join(self.tempdir, 'name.frontend.conf')))
...
""" % usrSharePath, open(join(self.tempdir, 'api.frontend.conf')).read())
    ...
    def testSslNoServices(self):
        n = NginxConfig(type='api', nginxConfigDir=self.tempdir, minVersion=VERSION, untilVersion=VERSION_PLUS_ONE, sslCertificate='/path/to/server.crt', sslKey='/path/to/server.pem')
        mustUpdate, sleeptime = n.updateConfig(
            services={},
            config={
                'api.frontend': {
...
""" % usrSharePath, open(join(self.tempdir, 'api.frontend.conf')).read())
    ...
    def testEndpointPortForListen(self):
        n = NginxConfig(type='api', nginxConfigDir=self.tempdir, minVersion=VERSION, untilVersion=VERSION_PLUS_ONE, endpoint='web')
        def endpoints(nr):
            result = {
        ...
        mustUpdate, sleeptime = n.updateConfig(
            services={
                ...
                newId(): {'type':'api', 'ipAddress':'10.0.0.5', 'infoport':1237, 'active':True, 'readable': True, 'data':{'VERSION': VERSION}},
            },
            config={
                'api.frontend': {
                    'port': 443,
