...
from meresco.distributed.failover import Proxy, ServiceConfig, UnusedConfig, SslConfig, HttpToHttpsRedirect, StaticServiceConfig, StaticServerName, StaticListenLine
from os import stat
...
class ProxyTest(SeecrTestCase):
    ...
    def testShouldNotWriteNginxConfigFileIfNothingChanged(self):
        ...
        self.assertEqual(stats, stat(join(self.tempdir, 'api.frontend.conf')))
...
upstream __var_dcac4f94de9ec18348d6b02350c45752_name {
...
server {
    ...
    location / {
        proxy_pass http://__var_dcac4f94de9ec18348d6b02350c45752_name;
    }
...
upstream __var_e9eb177b1f5eac5e64dbb271f55478a0_other {
    server 10.0.0.2:1234;
...
server {
    ...
    location / {
        proxy_pass http://__var_e9eb177b1f5eac5e64dbb271f55478a0_other;
    }
    error_page 500 502 503 504 =503 /unavailable.html;
    location /unavailable.html {
        root %s/failover;
...
upstream __var_d45a6b29e93ce0212ee61e79574f8953_api {
    server 10.0.0.3:2345;
}
...
server {
    ...
    location / {
        proxy_pass http://__var_d45a6b29e93ce0212ee61e79574f8953_api;
    }
    error_page 500 502 503 504 =503 /unavailable.html;
    location /unavailable.html {
        root %s/failover;
...
upstream __var_dcac4f94de9ec18348d6b02350c45752_api_18 {
...
server {
    ...
    location / {
        proxy_pass http://__var_dcac4f94de9ec18348d6b02350c45752_api_18;
    }
    location /sru {
        proxy_pass http://__var_dcac4f94de9ec18348d6b02350c45752_api_18;
...
""" % usrSharePath, read_from_file(self.configFile))
    ...
    def testUseAdminDownloadPeriod(self):
        ...
        self.assertEqual(5, sleeptime)

    def testListenIpsGlobalConfig(self):
        n1 = Proxy(nginxConfigFile=self.configFile)
        n1.addObserver(ServiceConfig(
                type='api',
                minVersion=VERSION,
...
upstream __var_dcac4f94de9ec18348d6b02350c45752_api {
    server 10.0.0.3:1235;
}
...
server {
    ...
    location / {
        proxy_pass http://__var_dcac4f94de9ec18348d6b02350c45752_api;
    }
    error_page 500 502 503 504 =503 /unavailable.html;
    location /unavailable.html {
        root %s/failover;
...
''', asString(c.sslLines()))
    ...
    def testServiceConfig(self):
        ...
        self.assertEqual(['api.front.example.org', 'alias1', 'alias2'], asList(c.servernames()))
        self.assertEqual('', asString(c.zones()))
        self.assertEqual('    location / {\n        proxy_pass http://__var_af9b2fd9f1c7f17413223dc3c26aeee4_api;\n    }', asString(c.locations()))
        self.assertEqual('    listen 0.0.0.0:80;\n', asString(c.listenLines()))
    ...
    def testServiceConfigListen(self):
        ...
        consume(c.updateConfig(**configServices))
        self.assertEqual('    listen 10.0.0.1:443;\n    listen 10.0.0.2:443;\n    listen 10.0.0.3:443;\n', asString(c.listenLines()))
    ...
    def testMatchingServices(self):
        ...
        consume(c.updateConfig(**CONFIG_SERVICES()))
        self.assertEqual([
                'upstream __var_af9b2fd9f1c7f17413223dc3c26aeee4_api {',
                '    server 10.0.0.2:1234;',
                '}',
                ''
    ...
    def testMultipleListenLines(self):
        ...
        p.addObserver(ServiceConfig(type='api', name='api1', minVersion="4.2", untilVersion="5.0", path='/other', port=443))
        p.addObserver(ServiceConfig(type='api', name='api2', minVersion="4.2", untilVersion="5.0", path='/main', port=443))
        self.assertEqual([
            '## Generated by meresco.distributed.failover.Proxy',
            '',
            'upstream __var_af9b2fd9f1c7f17413223dc3c26aeee4_api1 {',
            '    server 10.0.0.2:1234;',
            '}',
            'upstream __var_af9b2fd9f1c7f17413223dc3c26aeee4_api2 {',
            '    server 10.0.0.2:1234;',
            '}',
            '',
            'server {',
            ...
            '',
            '    location /other {',
            '        proxy_pass http://__var_af9b2fd9f1c7f17413223dc3c26aeee4_api1;',
            '    }',
            '    location /main {',
            '        proxy_pass http://__var_af9b2fd9f1c7f17413223dc3c26aeee4_api2;',
            '    }',
            '    error_page 500 502 503 504 =503 /unavailable.html;',
            '    location /unavailable.html {',
    ...
    def testPathAndPathsTypeError(self):
        self.assertRaises(TypeError, lambda: ServiceConfig(type='type', minVersion='1', untilVersion='2', path='/path', paths=['/other']))

    def testMultiplePathsToOneService(self):
        self.maxDiff = None
        p = Proxy(nginxConfigFile=self.configFile)
        p.addObserver(ServiceConfig(type='api', name='api', minVersion="4.2", untilVersion="5.0", paths=('/other', '/main'), port=443))
        self.assertEqual([
            '## Generated by meresco.distributed.failover.Proxy',
            '',
            'upstream __var_af9b2fd9f1c7f17413223dc3c26aeee4_api {',
    ...
    def testStaticServiceConfigWithMultiplePaths(self):
        self.maxDiff = None
        p = Proxy(nginxConfigFile=self.configFile)
        p.addObserver(StaticListenLine())
        p.addObserver(StaticServerName('api.front.example.org', 'alias1', 'alias2'))
        p.addObserver(StaticServiceConfig(paths=('/other', '/main'), remoteIp='1.2.3.4', remotePort=1234))
        p.addObserver(StaticServiceConfig(path='/somewhere/', remoteIp='1.2.3.4', remotePort=1234, remotePath="/", proxyHeaders=dict(Host="somewhere.completely.diff")))
        self.assertEqual([
            '## Generated by meresco.distributed.failover.Proxy',
            '',
            '',
            ...
            '    location /somewhere/ {',
            '        proxy_pass http://1.2.3.4:1234/;',
            '        proxy_set_header    Host somewhere.completely.diff;',
            '    }',
            '    error_page 500 502 503 504 =503 /unavailable.html;',
            '    location /unavailable.html {',
            '        root %s/failover;' % usrSharePath,
...
def grep(lines, text):
    return [x.strip() for x in [l for l in lines if text in l]]
